    ...
    {   
        ...
        {   
            ...
            for (size_t round = 0; round < 3; round++)
            {
                ...
                double cost = 0.0;
                for (size_t i : node->points) {
                    double old_dist = points_distance<with_weights>(data, d, i, node->index);
            ...
            for (size_t i : node->points)
            {
                if (points_distance<with_weights>(data, d, i, node->index) < points_distance<with_weights>(data, d, i, best_index))
            ...
            node->lc = std::make_unique<Node>(Node{
                ...
                .rc = nullptr,
                .parent = node,
                .cost = compute_costs(node->index, left_points),
                .index = node->index,
            ...
            node->rc = std::make_unique<Node>(Node{
                ...
                .rc = nullptr,
                .parent = node,
                .cost = compute_costs(best_index, right_points),
                .index = best_index,
...
static_assert(CoresetReducer<IndexCoresetReducer<0U, std::mt19937>>, "IndexCoresetReducer does not satisfy CoresetReducer concept");

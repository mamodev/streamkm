    ...
    inline Result __reduce(const float *const data, const std::size_t n, const std::size_t d, const std::size_t k)  {   
        ...
        {   
            ...
            for (size_t round = 0; round < 3; round++)
            {
                ...
                {
                    cumulative += flat_wpoints_l2_distance<WithWeights>(data, d, i, node.index);
                    if (cumulative >= prob)
                    {
                        random_point = i;
                        break;
                ...
                double cost = 0.0;
                for (size_t i : node.points) {
                    double old_dist = flat_wpoints_l2_distance<WithWeights>(data, d, i, node.index);
                    double new_dist = flat_wpoints_l2_distance<WithWeights>(data, d, i, random_point);
            ...
            for (size_t i : node.points) {   
                double old_dist = flat_wpoints_l2_distance<WithWeights>(data, d, i, node.index);
                double new_dist = flat_wpoints_l2_distance<WithWeights>(data, d, i, best_index);
...
static_assert(CoresetReducer<_IndexCoresetReducer<CoresetReducerNoMetrics, FenwickCoresetClusterSet>>, "IndexCoresetReducer does not satisfy CoresetReducer concept");

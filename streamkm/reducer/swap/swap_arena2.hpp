...
// This implementation add to SwapArenaCoresetReducer better 
// picking of new center to split by precomputing probabilities
// and the doing just one pass over cluster to pick center
...
struct SwapArena2CoresetReducer
{    
    ...
    static inline double points_distance(const float *const __restrict data, const size_t D, size_t a, size_t b)
    {
        double dist = 0.0;

        const float * __restrict pa = &data[a * (With_weights ? (D + 1) : D)];
        const float * __restrict pb = &data[b * (With_weights ? (D + 1) : D)];
...
static_assert(CoresetReducer<SwapArena2CoresetReducer<0U, std::mt19937>>, "SwapArena2CoresetReducer does not satisfy CoresetReducer concept");

...
struct IndexCachedCoresetReducer3
{    
    ...
    {   
        ...
        *root = {
        ...
        {   
            ...
            const size_t node_size = node->points.size();

            metrics.end_node_pick();
            metrics.set_node_size(node_size);

            passert(node_size > 1, "Node should have more than 1 point, got {}", node_size);
            ...
            size_t split_iters = std::min(MinSplitIters, node_size - 1);
            { // Initializing random picks
                ...
                if (p_index < split_iters) {
                    std::uniform_int_distribution<size_t> rand_index(0, node_size - 1);
                    for (; p_index < split_iters; p_index++) {
                        ...
                        random_pick_indices[p_index] = node->points[rp];
                    }
                }
            }
        ...
        while (!stack.empty())
        {
            ...
            if (node->is_leaf())
            {   
                size_t node_size = node->points.size();
                ...
                leaf_size_acc += node_size;
                leaf_count += 1;

                // sum with all points in the leaf
                float weight = 0;
                for (size_t pi = 0; pi < node_size; pi++) {
...
static_assert(CoresetReducer<IndexCachedCoresetReducer3<0U, std::mt19937>>, "IndexCachedCoresetReducer3 does not satisfy CoresetReducer concept");
